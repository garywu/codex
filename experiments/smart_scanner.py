...
class SmartScanner:
    ...
    def scan_file_with_refined_patterns(self, file_path: Path) -> list[dict]:
        ...
        for pattern_name, pattern_config in self.refined_patterns.items():
            pattern_violations = self.check_refined_pattern(pattern_config, file_path, content, lines)
            violations.extend(pattern_violations)

        return violations

    def check_refined_pattern(
        self, pattern_config: dict, file_path: Path, content: str, lines: list[str]
    ) -> list[dict]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if self.matches_trigger_patterns(line, pattern_config.get("triggers", [])):
                ...
                if pattern_config.get("context_required"):
                    if not self.has_required_context(content, pattern_config["context_required"]):
