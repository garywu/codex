...
class NegativeSpaceAnalyzer:
    ...
    def _is_python_project(self, project_dir: Path) -> bool:
        ...
        return any((project_dir / indicator).exists() for indicator in indicators) or bool(
    ...
    def _analyze_file_organization(self, project_dir: Path) -> dict[str, Any]:
        ...
        for py_file in py_files:
            try:
                ...
                if size > 10000:  # > 10KB
                    large_files.append({"file": str(py_file.relative_to(project_dir)), "size": size})
            except OSError:
                continue
            ...
            for project_name, project_data in all_projects_data.items():
                if pattern in project_data["violations_by_category"]:
                    ...
                    projects_without_problem.append(
                        {"project": project_name, "structure": project_data["project_structure"]}
                    )
        ...
        for pattern, data in negative_space_patterns.items():
            ...
            for factor in [
                ...
                if clean_rate - problem_rate > 0.3:
                    ...
                    logging.info(
                        f"   ✅ {factor}: Clean projects {clean_rate:.1%}, Problem projects {problem_rate:.1%}"
                    )
    ...
    def _create_negative_space_summary(
        self, negative_space_patterns: dict[str, Any], best_practices: list[dict[str, Any]]
    ) -> str:
