...
""",
    ...
    async def _check_tool(self, tool_name: str) -> ToolInfo:
        ...
        install_commands = {
            ...
            "typos": "uv pip install typos",  # or cargo install typos-cli
            "black": "uv pip install black",
            "uv": "pip install uv",  # Astral's fast package manager (bootstrap)
            "ty": "uv pip install ty",  # Astral's ultra-fast type checker
        }
    ...
    async def _run_command(self, cmd: list[str], cwd: Path | None = None) -> subprocess.CompletedProcess:
