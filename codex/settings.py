...
class CodexSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="CODEX_", env_file=".env", env_file_encoding="utf-8", case_sensitive=False, extra="ignore"
    )
    ...
    enable_fts: bool = Field(default=True, description="Enable SQLite FTS5 for AI queries")
    ...
    # Tool settings
    run_external_tools: bool = Field(default=True, description="Run external tools (ruff, mypy, typos)")

    tools_timeout: int = Field(default=120, description="Timeout for external tools in seconds")

    # AI settings
    ai_context_enabled: bool = Field(default=True, description="Enable AI context generation")

    ai_confidence_threshold: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Minimum confidence for AI suggestions"
    )

    # Pattern settings
    pattern_priorities: list[str] = Field(
        default=["MANDATORY", "CRITICAL", "HIGH"], description="Pattern priorities to enforce"
    )

    auto_fix: bool = Field(default=False, description="Automatically apply fixes")

    # Logging settings
    log_level: str = Field(default="INFO", description="Logging level")

    quiet_mode: bool = Field(default=False, description="Minimal output mode")

    # Farm SDK settings (optional)
    farm_url: str | None = Field(default=None, description="Farm SDK URL for AI agents")

    farm_enabled: bool = Field(default=False, description="Enable Farm SDK integration")

    # Repository settings
    repository_path: Path = Field(default=Path.cwd(), description="Current repository path")

    # Output settings
    output_format: str = Field(
        default="human", pattern="^(human|json|ai|markdown)$", description="Output format for results"
    )
