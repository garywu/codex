...
class NegativeSpaceDetector:
    ...
    def _extract_structural_features(self, project_path: Path) -> dict[str, Any]:
        ...
        features["has_pytest_config"] = (project_path / "pytest.ini").exists() or (
    ...
    def identify_negative_space_patterns(self, all_analyses: dict[str, ProjectAnalysis]) -> list[NegativeSpacePattern]:
        ...
        for violation_pattern in all_violation_patterns:
            ...
            if len(clean_projects) >= 1 and len(problematic_projects) >= 1:
                pattern = self._extract_protective_pattern(violation_pattern, clean_projects, problematic_projects)
                if pattern:
                    discovered_patterns.append(pattern)

        return discovered_patterns

    def _extract_protective_pattern(
        self, violation_pattern: str, clean_projects: list[ProjectAnalysis], problematic_projects: list[ProjectAnalysis]
    ) -> NegativeSpacePattern | None:
        ...
        for feature in [
            ...
            clean_rate = sum(1 for p in clean_projects if p.structural_features.get(feature, False)) / len(
                clean_projects
            )
            problem_rate = sum(1 for p in problematic_projects if p.structural_features.get(feature, False)) / len(
                problematic_projects
            )
        ...
        return NegativeSpacePattern(
            ...
            examples={"clean": [p.name for p in clean_projects], "problematic": [p.name for p in problematic_projects]},
        )
