...
async def handle_list_tools() -> list[types.Tool]:
    ...
    logfire.info("handle_list_tools called", dependencies_available=DEPENDENCIES_AVAILABLE, import_error=IMPORT_ERROR if not DEPENDENCIES_AVAILABLE else None)
        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "file_path": {
                        "type": "string",
                        "description": "Path to project-init.json file to import"
                    }
                },
                "required": ["file_path"]
            }
...
async def handle_call_tool(
    ...
    logfire.info("tool_call_received", tool_name=name, arguments=arguments, dependencies_available=DEPENDENCIES_AVAILABLE)
        ...
        if name == "query_patterns":
            ...
            if not patterns:
                return [types.TextContent(
                    type="text",
                    text=f"No patterns found for query: {query}"
                )]
            ...
            if not patterns:
                return [types.TextContent(
                    type="text", 
                    text=f"Pattern '{pattern_name}' not found in database."
                )]
            ...
            try:
                ...
                return [types.TextContent(
                    type="text",
                    text=f"✅ Successfully imported {imported_count} patterns from {file_path}"
                )]
            
            except Exception as e:
                return [types.TextContent(
                    type="text",
                    text=f"❌ Error importing patterns: {str(e)}"
                )]
...
async def main():
    ...
    logfire.info("mcp_server_starting", dependencies_available=DEPENDENCIES_AVAILABLE, server_name="codex-patterns", server_version="1.0.0")
