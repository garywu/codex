...
class InteractiveFixer:
    ...
    async def run_interactive_fix(self, target_path: Path, pattern_filter: str | None = None) -> int:
    ...
    async def _scan_for_violations(self, target_path: Path, pattern_filter: str | None = None) -> list[PatternMatch]:
            ...
            if match:
                ...
                if not old_name.startswith("mock_"):
                    ...
                    self.rollback_stack.append(
                        {"file": str(file_path), "line": violation.line_number - 1, "old": line, "new": new_line}
                    )
    ...
    async def _fix_uv_package_manager(self, violation: PatternMatch) -> bool:
        ...
        if modified:
            # Store rollback info
            self.rollback_stack.append({"file": str(file_path), "content": file_path.read_text()})
        ...
        if pattern_name == "structured-logging":
            if self.console and not self.auto_approve:
                log_format = Prompt.ask(
                    "Preferred log format", choices=["json", "structured", "console"], default="json"
                )
    ...
    async def _create_intelligent_fix_plan(self, pattern_name: str, violation: PatternMatch) -> FixPlan:
        ...
        file_path = Path(violation.file_path)
        if not file_path.exists():
            return FixPlan(violation=violation, strategy=FixStrategy.SKIP, explanation="File not found")
        ...
        return FixPlan(violation=violation, strategy=FixStrategy.MANUAL, explanation="Pattern requires manual analysis")
    ...
    def _analyze_cors_wildcard(self, violation: PatternMatch, context: list[str]) -> FixPlan:
        ...
        if "test" in violation.file_path.lower() or "dev" in violation.file_path.lower():
            return FixPlan(
                violation=violation, strategy=FixStrategy.SKIP, explanation="CORS wildcard acceptable in test/dev code"
            )
