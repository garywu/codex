...
from .models import AnalysisResult, CodeContext, PatternMatch, PatternCategory, PatternPriority
from .tools import ToolRunner
from .unified_database import UnifiedDatabase
from .negative_space_patterns import NegativeSpaceDetector
from .violation_reporter import ViolationReporter
from .scan_context import ScanContext, DecisionType
...
class Scanner:
    ...
    async def scan_file(self, file_path: Path) -> AnalysisResult:
        ...
        if self._is_excluded(file_path):
            self.scan_context.record_file_excluded(
                ...
                matched_pattern=self._get_exclusion_reason(file_path)
            )
            return AnalysisResult(file_path=str(file_path))
        ...
        try:
            ...
            self.scan_context.record_error(
                f"Reading file: {file_path.name}",
                str(e),
                file_path=file_path
            )
            return AnalysisResult(file_path=str(file_path))
        ...
        self.scan_context.record_file_included(
            file_path,
            f"File successfully loaded ({file_size} bytes)",
            file_size=file_size,
            language=language
        )
    ...
    def _is_excluded(self, file_path: Path) -> bool:
        ...
        default_excludes = [
            ...
            "*backup*", "build", "dist", "*.egg-info", "demo_repository"
        ]
    ...
    async def analyze_project_negative_space(self, project_path: Path) -> Dict[str, Any]:
        ...
        project_analysis = self.negative_space_detector.analyze_project_negative_space(
            project_path, violation_data
        )
    ...
    def _generate_negative_space_recommendations(self, project_analysis) -> List[str]:
        ...
        if not features.get('has_core_package'):
            recommendations.append(
                "📦 Create core/ package to separate business logic from interface layers"
            )
        
        if not features.get('has_api_package') and features.get('has_cli_package'):
            recommendations.append(
                "🔌 Add api/ package to enable multiple interface types (CLI, web, SDK)"
            )
        
        if not features.get('has_settings_file'):
            recommendations.append(
                "⚙️ Create unified settings module with Pydantic BaseSettings for configuration"
            )
        
        if not features.get('has_tests'):
            recommendations.append(
                "🧪 Add tests/ directory with proper pytest structure for quality assurance"
            )
        
        # Organization recommendations based on complexity indicators
        if features.get('package_depth', 0) > 5:
            recommendations.append(
                "📁 Reduce package nesting depth to 2-4 levels for better maintainability"
            )
        
        if features.get('has_monolithic_files'):
            recommendations.append(
                "📄 Break down large files (>15KB) into focused, single-responsibility modules"
            )
        
        # Security recommendations based on protective patterns
        if project_analysis.violations_by_pattern.get('hardcoded_secrets', 0) > 0:
            recommendations.append(
                "🔒 Implement environment variable pattern to avoid hardcoded secrets"
            )
        
        if project_analysis.violations_by_pattern.get('cors_never_wildcard', 0) > 0:
            recommendations.append(
                "🌐 Configure specific CORS origins instead of wildcard for security"
            )
