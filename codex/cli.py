...
def scan(
    paths: Annotated[
        list[Path] | None, typer.Argument(help="Files or directories to scan")
    ] = None,
    fix: Annotated[
        bool, typer.Option("--fix", "-f", help="Apply automatic fixes")
    ] = False,
    config: Annotated[
    ...
    diff: Annotated[
        bool, typer.Option("--diff", help="Show diff of proposed fixes")
    ] = False,
    exclude: Annotated[
        str | None, typer.Option("--exclude", help="Glob pattern to exclude")
    ] = None,
    no_tools: Annotated[
        bool, typer.Option("--no-tools", help="Skip external tools (ruff, mypy, typos)")
    ] = False,
    best_practices: Annotated[
        bool, typer.Option("--best-practices", "-bp", help="Include negative space analysis for evidence-based best practices")
    ] = False,
    ...
    list_files: Annotated[
        bool, typer.Option("--list-files", help="List all files that would be scanned")
    ] = False,
    explain: Annotated[
        bool, typer.Option("--explain", help="Verbose mode explaining every decision")
    ] = False,
) -> None:
    ...
    async def _scan() -> int:
                ...
                if path.is_dir():
                    ...
                    if explain:
                        ...
                        for decision in discovery_result.excluded_files[:20]:  # Show first 20
                            reason_key = f"{decision.reason} ({decision.matched_pattern})" if decision.matched_pattern else decision.reason
                ...
                if best_practices:
                    try:
                        ...
                        if not quiet and 'error' not in negative_space_results:
                            ...
                            logging.info(f"Excellence Level: {negative_space_results.get('excellence_level', 'Unknown')}")
                            logging.info(f"Organization Score: {negative_space_results.get('organization_score', 0):.1%}")
        ...
        if json_output or export_json or export_audit:
            ...
            report = violation_reporter.generate_report(scan_root, scan_context.total_duration_ms if scan_context else 0.0)
...
def fix_interactive(
    paths: Annotated[
        list[Path] | None, typer.Argument(help="Files or directories to fix")
    ] = None,
    pattern: Annotated[
        str | None, typer.Option("--pattern", "-p", help="Fix only specific pattern")
    ] = None,
    auto: Annotated[
        bool, typer.Option("--auto", "-a", help="Auto-approve all safe fixes")
    ] = False,
    rollback: Annotated[
        bool, typer.Option("--rollback", "-r", help="Rollback previous fix session")
    ] = False,
    quiet: Annotated[
...
def fix_safe(
    ...
    impact: Annotated[
        bool, typer.Option("--impact", "-i", help="Show impact analysis before fixing")
    ] = False,
    audit_report: Annotated[
        Path | None, typer.Option("--audit-report", help="Export audit report to file")
    ] = None,
) -> None:
    """
    ...
    """
    from .fix_orchestrator import FixOrchestrator, FixMode
    from .scanner import Scanner
...
def init(
    import_from: Annotated[
        Path | None, typer.Option("--import", help="Import patterns from file")
    ] = None,
    farm_url: Annotated[
        str | None, typer.Option("--farm-url", help="Farm SDK URL")
    ] = "http://localhost:8001",
) -> None:
...
def check(
    ...
    paths: Annotated[
        list[Path] | None, typer.Argument(help="Files to check")
    ] = None,
) -> None:
...
def patterns(
    list_all: Annotated[
        bool, typer.Option("--list", "-l", help="List all patterns")
    ] = False,
    category: Annotated[
        PatternCategory | None, typer.Option("--category", "-c")
    ] = None,
    add: Annotated[
        Path | None, typer.Option("--add", "-a", help="Add patterns from file")
    ] = None,
) -> None:
...
def query(
    ...
    limit: Annotated[
        int, typer.Option("--limit", "-l", help="Maximum patterns to return")
    ] = 5,
    ai_format: Annotated[
        bool, typer.Option("--ai", help="Format output for AI consumption")
    ] = False,
    priority: Annotated[
    ...
    if ai_format:
        ...
        for i, pattern in enumerate(result['patterns'], 1):
            ...
            console.logging.info(f"[bold]{i}. [{priority_color}]{pattern['priority']}[/{priority_color}] {pattern['name']}[/bold]")
...
def context(
    file_path: Annotated[
        str | None, typer.Option("--file", "-f", help="File path for context")
    ] = None,
    intent: Annotated[
        str | None, typer.Option("--intent", "-i", help="Coding intent")
    ] = None,
    category: Annotated[
        str | None, typer.Option("--category", "-c", help="Pattern category")
    ] = None,
    ai_format: Annotated[
        bool, typer.Option("--ai", help="Format output for AI consumption")
    ] = False,
    max_patterns: Annotated[
        int, typer.Option("--limit", "-l", help="Maximum patterns to return")
    ] = 10,
) -> None:
    ...
    if file_path:
        ...
        for pattern in patterns:
            ...
            console.logging.info(f"[{priority_color}]{pattern['priority']:10}[/{priority_color}] "
...
def validate(
    code_file: Annotated[
        Path | None, typer.Argument(help="File to validate")
    ] = None,
    code_text: Annotated[
        str | None, typer.Option("--code", "-c", help="Code snippet to validate")
    ] = None,
    language: Annotated[
        str, typer.Option("--language", "-l", help="Programming language")
    ] = "python",
) -> None:
    ...
    if result['violations']:
        ...
        for i, violation in enumerate(result['violations'], 1):
            ...
            console.logging.info(f"[bold]{i}. [{priority_color}]{violation['priority']}[/{priority_color}] {violation['pattern']}[/bold]")
...
def import_patterns(
    ...
    db_path: Annotated[
        str | None, typer.Option("--db", help="Database path")
    ] = None,
) -> None:
...
def export(
    format: Annotated[
        str, typer.Option("--format", "-f", help="Export format (markdown, json)")
    ] = "markdown",
    output: Annotated[
...
def serve(
    mcp: Annotated[
        bool, typer.Option("--mcp", help="Run as MCP server")
    ] = False,
    stdio: Annotated[
        bool, typer.Option("--stdio", help="Use stdio transport")
    ] = True,
) -> None:
...
def install_startup(
    system: Annotated[
        bool, typer.Option("--system", help="Install as system service (requires sudo)")
    ] = False,
    user: Annotated[
        bool, typer.Option("--user", help="Install as user service")
    ] = True,
) -> None:
        ...
        try:
            ...
            if system:
                result = subprocess.run(
                    ["sudo", "launchctl", "load", str(plist_dest)],
...
def uninstall_startup(
    system: Annotated[
        bool, typer.Option("--system", help="Remove system service (requires sudo)")
    ] = False,
) -> None:
...
def startup_status() -> None:
    ...
    if platform.system() == "Darwin":
        ...
        if system_plist.exists():
            ...
            try:
                result = subprocess.run(
                    ["sudo", "launchctl", "print", "system/com.codex.mcp-server"],
                    capture_output=True, text=True
                )
                if result.returncode == 0:
        ...
        if user_plist.exists():
            ...
            try:
                result = subprocess.run(
                    ["launchctl", "print", f"gui/{os.getuid()}/com.codex.mcp-server"],
                    capture_output=True, text=True
                )
                if result.returncode == 0:
                    console.logging.info("Status: 🟢 Running")
        ...
        if system_service.exists():
            ...
            try:
                result = subprocess.run(
                    ["systemctl", "is-active", "codex-mcp.service"],
                    capture_output=True, text=True
                )
        ...
        if user_service.exists():
            ...
            try:
                result = subprocess.run(
                    ["systemctl", "--user", "is-active", "codex-mcp.service"],
                    capture_output=True, text=True
                )
                status = result.stdout.strip()
                if status == "active":
...
def portable(
    target_dir: Annotated[
        Path | None, typer.Argument(help="Target directory to scan")
    ] = None,
    fix: Annotated[
        bool, typer.Option("--fix", "-f", help="Apply automatic fixes")
    ] = False,
    ...
    no_config: Annotated[
        bool, typer.Option("--no-config", help="Don't generate default configs")
    ] = False,
    quiet: Annotated[
        bool, typer.Option("--quiet", "-q", help="Minimal output")
    ] = False,
...
def init_repo(
    repo_path: Annotated[
        Path | None, typer.Argument(help="Repository path to initialize")
    ] = None,
    patterns: Annotated[
        list[Path] | None, typer.Option("--patterns", "-p", help="Pattern files to import")
    ] = None,
    precommit: Annotated[
        bool, typer.Option("--precommit/--no-precommit", help="Set up pre-commit hooks")
    ] = True,
    tools: Annotated[
        bool, typer.Option("--tools/--no-tools", help="Set up quality tools")
    ] = True,
    quiet: Annotated[
        bool, typer.Option("--quiet", "-q", help="Minimal output")
    ] = False,
...
def tools_command(
    check: Annotated[
        bool, typer.Option("--check", "-c", help="Check tool availability")
    ] = False,
    install: Annotated[
        str | None, typer.Option("--install", "-i", help="Install specific tool")
    ] = None,
    config: Annotated[
        str | None, typer.Option("--config", help="Generate config for tool")
    ] = None,
    target_dir: Annotated[
        Path | None, typer.Option("--dir", "-d", help="Target directory")
    ] = None,
) -> None:
    """
...
def any_repo(
    ...
    init: Annotated[
        bool, typer.Option("--init", help="Initialize repository first")
    ] = False,
    patterns: Annotated[
        Path | None, typer.Option("--patterns", "-p", help="Pattern file to use")
    ] = None,
    quiet: Annotated[
        bool, typer.Option("--quiet", "-q", help="Minimal output")
    ] = False,
...
def scan_to_database(
    paths: Annotated[
        list[Path] | None, typer.Argument(help="Paths to scan")
    ] = None,
    output_db: Annotated[
        Path, typer.Option("--output-db", "-o", help="Output database path")
    ] = Path("scan_results.db"),
    ai_context: Annotated[
        str | None, typer.Option("--ai-context", help="AI context for scan")
    ] = None,
    quiet: Annotated[
        bool, typer.Option("--quiet", "-q", help="Minimal output")
    ] = False,
...
def query_database(
    ...
    ai_format: Annotated[
        bool, typer.Option("--ai", help="AI-optimized output")
    ] = False,
    explain: Annotated[
        bool, typer.Option("--explain", help="Show SQL and explanation")
    ] = False,
    limit: Annotated[
...
def config_cmd(
    action: Annotated[
        str, typer.Argument(help="Action: init, show, set, get, clean")
    ],
    key: Annotated[
        str | None, typer.Argument(help="Configuration key for get/set")
    ] = None,
    value: Annotated[
        str | None, typer.Option("--value", "-v", help="Value to set")
    ] = None,
    template: Annotated[
        str, typer.Option("--template", "-t", help="Template for init")
    ] = "python",
    project_root: Annotated[
        Path | None, typer.Option("--project", "-p", help="Project root directory")
    ] = None,
) -> None:
...
def install_mcp(
    user_scope: Annotated[bool, typer.Option("--user/--project", help="Install for current user vs project only")] = True,
    ...
    if user_scope:
        ...
        if not force:
            try:
                result = subprocess.run(["claude", "mcp", "get", "codex"], 
                                      capture_output=True, text=True)
        ...
        if force:
            try:
                subprocess.run(["claude", "mcp", "remove", "codex", "-s", "user"], 
                             capture_output=True, text=True)
            except:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                config_result = subprocess.run(["claude", "mcp", "get", "codex"], 
                                             capture_output=True, text=True)
...
def mcp_status():
    ...
    try:
        import subprocess
        result = subprocess.run([codex_path, "mcp-server", "--help"], 
...
def learn_pattern(
    ...
    rule: Annotated[str, typer.Argument(help="Rule description (e.g., 'Use pathlib instead of os.path for file operations')")],
    ...
    source: Annotated[str, typer.Option("--source", help="Source of pattern (e.g., 'claude-3.5-sonnet')")] = "ai-assistant",
    ...
    try:
        ...
        ''', (
            ...
            pattern['priority'], pattern.get('detection_pattern'), pattern.get('fix_template'), 
            pattern['rationale'], pattern.get('example_good'), pattern.get('example_bad'),
            pattern['source_file'], pattern['full_context']
        ))
