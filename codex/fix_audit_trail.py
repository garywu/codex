...
class FixAuditTrail:
    ...
    def record_decision(self, audit_id: str, decision: FixDecision, reason: str, user_id: str | None = None):
    ...
    def record_application(self, audit_id: str, file_hash_after: str, execution_time_ms: float, lines_changed: int):
        ...
        cursor.execute(
            ...
            WHERE audit_id = ?
        """,
            (FixStatus.APPLIED, file_hash_after, execution_time_ms, lines_changed, audit_id),
        )
    ...
    def record_rollback(self, audit_id: str, reason: str):
        ...
        cursor.execute(
            ...
            WHERE audit_id = ?
        """,
            (FixStatus.ROLLED_BACK, datetime.now().isoformat(), reason, audit_id),
        )
    ...
    def get_session_summary(self, session_id: str | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                status_counts.get(FixStatus.APPLIED, 0) / sum(status_counts.values()) * 100 if status_counts else 0
            ),
        }
    ...
    def export_audit_report(
        self, output_path: Path, start_date: datetime | None = None, end_date: datetime | None = None
    ):
    ...
    def learn_from_history(self) -> dict[str, Any]:
        ...
        risky_patterns = [{"pattern": row[0], "failure_rate": row[1] / row[2] * 100} for row in cursor.fetchall()]
        ...
        rollback_patterns = [{"pattern": row[0], "rollback_count": row[1]} for row in cursor.fetchall()]
        ...
        slow_patterns = [{"pattern": row[0], "avg_time_ms": row[1]} for row in cursor.fetchall()]
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(risky_patterns, rollback_patterns, slow_patterns),
        }

    def _generate_recommendations(
        self, risky_patterns: list[dict], rollback_patterns: list[dict], slow_patterns: list[dict]
    ) -> list[str]:
...
if __name__ == "__main__":
    ...
    audit_trail.update_validation_results(
        audit_id=audit_id, syntax_valid=True, imports_valid=True, tests_passed=True, validation_errors=[]
    )

    # Record decision
    audit_trail.record_decision(audit_id=audit_id, decision=FixDecision.AUTO_APPROVED, reason="All validations passed")
    ...
    audit_trail.record_application(
        audit_id=audit_id, file_hash_after="abc123def456", execution_time_ms=execution_time, lines_changed=1
    )
