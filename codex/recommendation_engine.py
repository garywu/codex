...
class ProjectArchitectureAnalyzer:
    ...
    def _analyze_pydantic_models_opportunity(self, context: dict[str, Any]) -> list[TechRecommendation]:
        ...
        has_manual_validation = context["has_data_validation"] and (
            "manual_type_checking" in context["validation_patterns"]
            or "dict_parameters" in context["validation_patterns"]
        )
        ...
        if has_manual_validation and context["has_api_endpoints"]:
            ...
            recommendations.append(
                TechRecommendation(
                    ...
                    evidence=["Manual type checking patterns found", "Dict-based data handling"],
                    priority="MEDIUM",
                    implementation_effort="20-30 minutes",
    ...
    def _analyze_logfire_opportunity(self, context: dict[str, Any]) -> list[TechRecommendation]:
        ...
        has_structured_logging_need = context["has_logging"] and (
            "formatted_print_statements" in context["logging_patterns"]
            or "basic_logging" in context["logging_patterns"]
        )
    ...
    def print_recommendations(self, recommendations: list[TechRecommendation]) -> None:
        ...
        if not self.quiet:
            self.console.print("\n")
            self.console.print(Panel.fit("🚀 Technology Recommendations", style="bold blue"))

            for rec in sorted(
                recommendations, key=lambda x: {"HIGH": 3, "MEDIUM": 2, "LOW": 1}[x.priority], reverse=True
            ):
