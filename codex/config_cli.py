...
def validate():
        ...
        for pattern in exclude_patterns:
            if "*_backup*" in pattern:
                ...
                if backup_dirs:
                    ...
                    warnings.append(f"Pattern '{pattern}' will exclude {len(backup_dirs)} directories ({file_count} files)")
    ...
    if config.get("run_tools", True):
        ...
        if missing_tools:
            ...
            warnings.append(f"Install missing tools with: pip install {' '.join(missing_tools)}")
