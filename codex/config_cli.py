...
def sources():
    ...
    sources_info.append(("Project config (.codex.toml)", codex_toml.exists(), str(codex_toml)))
    ...
    sources_info.append(("Project config (pyproject.toml [tool.codex])", has_codex_section, str(pyproject)))

    # Environment variables
    env_vars = [k for k in os.environ if k.startswith("CODEX_")]
    sources_info.append(("Environment variables", len(env_vars) > 0, ", ".join(env_vars) if env_vars else "none"))
...
def _save_codex_toml(config: dict[str, Any]):
    ...
    config_with_comments = """# Codex Configuration File
