...
class FixContextAnalyzer:
    ...
    def analyze_violation_context(self, file_path: Path, line_number: int, window_size: int = 10) -> CodeContext:
        """
        ...
        try:
            ...
            return CodeContext(file_path=file_path, line_number=line_number, is_test_code=self._is_test_file(file_path))
    ...
    def _get_enclosing_function(self, node: ast.AST, tree: ast.AST) -> str | None:
    ...
    def _get_enclosing_class(self, node: ast.AST, tree: ast.AST) -> str | None:
    ...
    def _analyze_dependencies(self, content: str, line_number: int, window_size: int) -> set[str]:
    ...
    def is_fix_safe_in_context(self, context: CodeContext, fix_type: str) -> tuple[bool, str]:
        """
    ...
    def suggest_fix_adjustments(self, context: CodeContext, original_fix: str) -> str:
        """
...
class FixConflictDetector:
    ...
    def detect_conflicts(self, fixes: list[dict[str, Any]]) -> list[tuple[int, int, str]]:
        """
    ...
    def _check_conflict(self, fix1: dict, fix2: dict) -> str | None:
        ...
        if file1 == file2:
            ...
            if pattern1 in ["import-order", "unused-import"] and pattern2 in ["import-order", "unused-import"]:
                return "Both fixes modify imports"

            # Both modify function signature
            if pattern1 in ["function-signature", "type-hints"] and pattern2 in ["function-signature", "type-hints"]:
                return "Both fixes modify function signature"

        return None

    def resolve_conflicts(
        self, fixes: list[dict[str, Any]], conflicts: list[tuple[int, int, str]]
    ) -> list[dict[str, Any]]:
        """
