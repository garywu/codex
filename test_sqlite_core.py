...
def populate_test_data(db):
    ...
    for file_path, file_type, file_size, line_count, complexity in files_data:
        ...
        """, (
            file_id, session_id, file_path, file_type, file_size,
            line_count, complexity, datetime.now(), 50
        ))
    ...
    for violation in violations_data:
        ...
        """, (
            violation_id, session_id, violation["file_id"], violation["pattern_name"],
            ...
            violation["message"], violation["ai_explanation"], violation["fix_complexity"],
            json.dumps([]), violation["business_impact"], violation["tags"]
        ))
        ...
        try:
            ...
            """, (
                violation_id, violation["pattern_name"], violation["category"],
                violation["code_snippet"], violation["message"], violation["ai_explanation"],
                "test_file.py", violation["tags"]
            ))
        except:
    ...
    for insight in insights_data:
        ...
        """, (
            insight_id, session_id, insight["type"], insight["category"],
            insight["confidence"], insight["title"], insight["description"],
            json.dumps({}), insight["impact"], json.dumps(insight["recommendations"])
        ))
        ...
        try:
            ...
            """, (
                insight_id, insight["type"], insight["title"], 
...
def test_ai_queries(db):
    ...
    for row in results:
        print(f"  • {row['severity']} {row['pattern_category']}: {row['count']} violations (confidence: {row['avg_confidence']})")
    ...
    for row in results:
        print(f"  • {row['file_path']}: {row['violation_count']} violations (avg severity: {row['avg_severity_score']:.1f})")
