...
class HardcodedPathsFixer:
    ...
    def needs_settings_import(self, content: str) -> bool:
        ...
        uses_settings = any(replacement.startswith('settings.') 
                          for replacement in self.path_replacements.values())
        has_settings = any(line in content for line in [
    ...
    def fix_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if needs_settings and self.needs_settings_import(content):
                ...
                file_fixes.append({
                    'type': 'import_added',
                    'description': 'Added settings import'
                })
