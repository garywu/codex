...
class BestPracticesScanner:
    ...
    def _find_pattern_matches(self, file_analysis: Dict[str, Dict[str, Any]], pattern: Dict[str, Any]) -> List[Dict[str, Any]]:
...
def main():
    ...
    for project_dir in work_dir.iterdir():
        if project_dir.is_dir() and not project_dir.name.startswith('.'):
            ...
            if any((project_dir / indicator).exists() for indicator in 
                   ['pyproject.toml', 'setup.py', 'requirements.txt']) or \
    ...
    if all_results:
        ...
        # Excellence leaders
        project_scores = [(r['project_name'], r['excellence_metrics']['overall_score']) 
