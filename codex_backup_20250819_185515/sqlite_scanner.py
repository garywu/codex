...
class SQLiteScanner:
    ...
    async def scan_repository(self, repo_path: Path) -> str:
        ...
        if not self.quiet:
            self.console.logging.info(f"[green]✅ Scan complete: {files_scanned} files, {total_violations} violations[/green]")
    ...
    def _generate_ai_explanation(self, violation: PatternMatch) -> str:
        ...
        return explanations.get(
            violation.pattern_name,
            f"Pattern violation detected: {violation.suggestion}"
        )
    ...
    async def _generate_repository_insights(self) -> list[dict[str, Any]]:
        ...
        if violation_stats:
            insights.append({
                ...
                "supporting_evidence": {
                    "categories": [dict(row) for row in violation_stats]
                },
        ...
        if large_files:
            insights.append({
                ...
                "supporting_evidence": {
                    "largest_files": [dict(row) for row in large_files]
                },
