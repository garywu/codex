    ...
    if format == OutputFormat.JSON:
        ...
        for p in patterns:
            ...
            table.add_row(
                ...
                p.get("description", "")[:60] + "..." if len(p.get("description", "")) > 60 else p.get("description", "")
            )
...
def search(
    ...
    if format == OutputFormat.JSON:
        ...
        for p in patterns:
            ...
            data_console.logging.info(f"  [dim]Category: {p.get('category', 'N/A')} | Priority: {p.get('priority', 'N/A')}[/dim]")
...
def bulk(
    ...
    async def run_bulk_operation():
        ...
        if ai_query or operation == "query":
            ...
            categories = ["error_handling", "naming", "validation", "logging", "dependencies", "testing", "imports", "organization", "git"]
            ...
            return {
                "query": query_text,
                "count": len(results),
                "patterns": [{"name": r["name"], "category": r.get("category"), "priority": r.get("priority"), "description": r.get("description")} for r in results]
            }
        ...
        if any(word in prompt_lower for word in ["what", "show", "list", "find", "search"]):
            ...
            if context_data:
                ...
                if execute:
                    ...
                    patterns_data = context_data.get("patterns", context_data if isinstance(context_data, list) else [context_data])
                    missing = []
