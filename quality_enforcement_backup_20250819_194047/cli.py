...
def scan(
    ...
    def _scan() -> int:
        # Use unified database for pattern matching
        db = UnifiedDatabase()
        violations = 0
        ...
        if not patterns:
            if not quiet:
                console.logging.info("[yellow]No patterns found in database. Run 'codex init' first.[/yellow]")
            return 0
        
        # Debug info removed for clean operation
        
        # Initialize project config for gitignore support
        project_config = ProjectConfigManager()
        gitignore_handler = GitIgnoreHandler(project_config.project_root)
        
        for path in paths:
            if path.is_file():
                if gitignore_handler.should_exclude(path):
                    continue
                    
                # Simple file scanning - check for basic pattern violations
                try:
                    ...
                    for pattern in patterns:  # Check all patterns
                        ...
                        keywords_to_check = []
                        if hasattr(pattern, 'detection') and pattern.detection:
                            keywords_to_check.extend(pattern.detection.keywords)
                        ...
                        for keyword in keywords_to_check:
                            if keyword in content:
                                file_violations += 1
                                if not quiet:
                                    console.logging.info(f"[red]✗[/red] {path}: {pattern.name} - {pattern.rule}")
                    ...
                    if not quiet:
                        console.logging.info(f"[yellow]Warning: Could not read {path}[/yellow]")
                        
            elif path.is_dir():
                # Get scannable files using gitignore handler
                scannable_files = gitignore_handler.get_scannable_files(path)
                for file_path in scannable_files[:10]:  # Limit for demo
                    try:
                        ...
                        # Check patterns against file content
                        file_violations = 0
                        for pattern in patterns[:5]:  # Limit to first 5 patterns for demo
                            # Handle Pattern objects
                            keywords_to_check = []
                            if hasattr(pattern, 'detection') and pattern.detection:
                                keywords_to_check.extend(pattern.detection.keywords)
                            ...
                            for keyword in keywords_to_check:
                                if keyword in content:
                                    file_violations += 1
                                    if not quiet:
                                        console.logging.info(f"[red]✗[/red] {file_path}: {pattern.name} - {pattern.rule}")
                                    break  # Only count once per pattern
        ...
        if violations > 0:
            if not quiet:
                console.logging.info(f"\n[red]Found {violations} violation(s)[/red]")
            return 1
        else:
            if not quiet:
