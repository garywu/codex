...
def show_detailed_violations():
    ...
    filtered_out = [v for v in raw_violations if not any(
        fp['violation'].get('file') == v.get('file') and 
        fp['violation'].get('line') == v.get('line')
    ...
    critical_count = sum(1 for fp in results.get('fix_plans', []) 
                        if fp.get('violation', {}).get('priority') == 'CRITICAL')
    mandatory_count = sum(1 for fp in results.get('fix_plans', []) 
                         if fp.get('violation', {}).get('priority') == 'MANDATORY')
    ...
    high_count = sum(1 for fp in results.get('fix_plans', []) 
                    if fp.get('violation', {}).get('priority') == 'HIGH')
    if high_count > 0:
